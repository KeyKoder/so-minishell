	...
	while (fgets(buf, 1024, stdin)) {
		...
		if (line->ncommands != 0) {
			...
			if (line->redirect_output != NULL) {
				currentJob->stdoutRedirect = fopen(line->redirect_output, "w");
			}
			if (line->redirect_error != NULL) {
				currentJob->stderrRedirect = fopen(line->redirect_error, "w");
			}
			if (line->background) {
					...
					for (i = 0; i < currentJob->line->ncommands; i++) {
						...
						if (currentJob->childPid == 0) {  // JOB CHILD (single command execution)
							if (i == 0 && currentJob->line->redirect_input != NULL) {
								close(STDIN_FILENO);
								dup(fileno(currentJob->stdinRedirect));
								fclose(currentJob->stdinRedirect);
							}
							if (i == currentJob->line->ncommands - 1 && currentJob->line->redirect_output != NULL) {
								close(STDOUT_FILENO);
								dup(fileno(currentJob->stdoutRedirect));
								fclose(currentJob->stdoutRedirect);
							}
							if (i == currentJob->line->ncommands - 1 && currentJob->line->redirect_error != NULL) {
								close(STDERR_FILENO);
								dup(fileno(currentJob->stderrRedirect));
...
job_t* createJob() {
	...
	job->stdoutRedirect = NULL;
	job->stderrRedirect = NULL;
...
void freeJob(job_t* job) {
	...
	if (job->stdoutRedirect != NULL) {
		fclose(job->stdoutRedirect);
		job->stdoutRedirect = NULL;
	}
	if (job->stderrRedirect != NULL) {
		fclose(job->stderrRedirect);
		job->stderrRedirect = NULL;
	}
